<x-app-layout>
    <div class="max-w-4xl mx-auto p-6">
        <h1 class="text-2xl font-semibold mb-4">Tambah Kamar</h1>
        ...
        <form method="POST" action="{{ route('admin.rooms.store') }}" enctype="multipart/form-data" class="space-y-4">
            @csrf
            ...
            <div class="grid md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="number" name="capacity" min="1" value="{{ old('capacity',2) }}" class="w-full border rounded px-3 py-2" required>
                </div>
                <div>
                    <label class="block text-sm mb-1">Harga per Malam</label>
                    <input type="number" name="price_per_night" min="0" value="{{ old('price_per_night') }}" class="w-full border rounded px-3 py-2" required>
                </div>
            </div>
            ...
            <div class="flex gap-3">
                ...
                <a href="{{ route('admin.rooms.index') }}" class="px-4 py-2 rounded bg-gray-600 text-white">Batal</a>
            </div>
        </form>
    </div>
</x-app-layout>
